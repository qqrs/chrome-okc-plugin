...
$('<span class="okcp-improved-link">Improved!</span>')
  .appendTo('#logo')
  .click(function() {
    window.open(
      'http://chrome.google.com/webstore/detail/cgdblghohnaeeejaoincmbcdkdnodkei/',
      '_blank',
    );
  });
...
elt.innerHTML =
  'const accessTokenGetterTimout = setInterval(() => {if (window.ACCESS_TOKEN) {document.head.id = window.ACCESS_TOKEN; clearInterval(accessTokenGetterTimout)}}, 1000)';
...
(function() {
  ...
  storage.questionCategories = storage.questionCategories || [
    'not_volatile',
    'generally_happy',
    'non-monogamous',
    'communicative',
  ...
  for (var i = 0; i < deprecatedKeys.length; i++) {
    if (!!localStorage[deprecatedKeys[i]]) {
      ...
      upgradeMessage.push(
        '  * Removed deprecated key (' + deprecatedKeys[i] + ')',
      );
    }
  }
...
$(document).ready(() => {
  ...
  // $("#save_unsave")
  //   .parent()
  //   .addClass("wide-buttons-that-are-now-not-wide left")
  //   .next()
  //   .addClass("wide-buttons-that-are-now-not-wide right");

  // Change OkCupid's UI
  if ($('#visit_button a:contains(Enable invisible browsing)').size() > 0) {
    $('#visit_button')
      .addClass('moved')
      .insertAfter($('#similar_users_list'));
  }
  $('#profile_ad').hide();
